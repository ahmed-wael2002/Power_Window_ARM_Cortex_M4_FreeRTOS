...
uint8 DIO_readPort(uint8 PortNumber){
    ...
    if (PortNumber >= NUM_OF_PORTS){
        ...
        switch(PortNumber){
            case PORTA_ID:
                port_value = (uint8)GPIO_PORTA_DATA_R;
                break;

            case PORTB_ID:
                port_value = (uint8)GPIO_PORTB_DATA_R;
                break;

            case PORTC_ID:
                port_value = (uint8)GPIO_PORTC_DATA_R;
                break;

            case PORTD_ID:
                port_value = (uint8)GPIO_PORTD_DATA_R;
                break;

            case PORTE_ID:
                port_value = (uint8)GPIO_PORTE_DATA_R;
                break;

            case PORTF_ID:
                port_value = (uint8)GPIO_PORTF_DATA_R;
                break;
        }
        return port_value;
    }
